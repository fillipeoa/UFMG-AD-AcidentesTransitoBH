...
def gerar_csv_limpo():
    data = pd.read_csv('datasets/acidentes_veiculos.csv', low_memory=False)
    ...
    data.to_csv('datasets/acidentes_veiculos_limpo.csv', index=False, sep='\t')

def gerar_entradas_acidentes_ocorridos():
    dados = pd.read_csv('datasets/acidentes_veiculos_limpo.csv', low_memory=False, sep='\t')
    ...
    properties.to_csv('datasets/ocorridos/properties.csv', index=False, sep='\t')

    # GERAR ARQUIVO QUALITIES
    qualities = pd.DataFrame([1]*len(properties), columns=['OCORRIDO'])
    qualities.to_csv('datasets/ocorridos/qualities.csv', index=False, sep='\t')
...
def gerar_entradas_acidentes_ocorridos_centro_sul():
    ...
    #Remova todas as linhas que tem alguma das colunas filtradas com valor NÃO INFORMADO
    #properties = properties[~properties.isin(['NÃO INFORMADO','NAO INFORMADO']).any(axis=1)]
    properties.to_csv('datasets/ocorridosCentroSul/properties.csv', index=False, sep='\t')

    # GERAR ARQUIVO QUALITIES
    qualities = dados[['CENTRO-SUL']]
    qualities.to_csv('datasets/ocorridosCentroSul/qualities.csv', index=False, sep='\t')
...
def gerar_entradas_acidentes_ocorridos_automoveis():
    ...
    qualities.to_csv('datasets/ocorridosAutomoveis/qualities.csv', index=False, sep='\t')
...
def gerar_entradas_acidentes_ocorridos_pico_tarde():
    ...
    #Remova todas as linhas que tem alguma das colunas filtradas com valor NÃO INFORMADO
    #properties = properties[~properties.isin(['NÃO INFORMADO','NAO INFORMADO']).any(axis=1)]
    properties.to_csv('datasets/ocorridosPicoTarde/properties.csv', index=False, sep='\t')

    # GERAR ARQUIVO QUALITIES
    qualities = dados[['PICO_TARDE']]
    qualities.to_csv('datasets/ocorridosPicoTarde/qualities.csv', index=False, sep='\t')
